...
const Navbar = () => {
    ...
    return (
        <AnimatePresence>
            {isVisible && (
                <motion.nav className={`shadow-xl z-50 fixed top-0 right-0 left-0 p-2 ${isScrollingUp ? 'bg-white' : 'bg-transparent'}`} variants={navLinksVariants}>
                    <div className="flex justify-between items-center">
                        <div>
                            <Link className='flex items-center' to="/">
                                <img className='w-12 h-12 sm:w-16 sm:h-16 lg:w-20 lg:h-20' src={logo} alt="" />
                                <div className='ml-2 sm:ml-3'>
                                    <p className='text-black text-left text-lg sm:text-base lg:text-xl xl:text-2xl font-bold'>Parcelin Company</p>
                                </div>
                            </Link>
                        </div>
                        {isMobileView ? (
                            ...
                            <div className="hidden lg:flex gap-4 xl:gap-8 items-center">
                                {navLinks.map((link, index) => (
                                    <Link key={index} to={link.to} className={`text-black text-sm lg:text-base xl:text-lg font-semibold hover:scale-110 duration-200 delay-150 ${activePage && activePage.to === link.to ? 'border-b-2 border-black' : ''}`}>
                                        {link.title}
                                    </Link>
                                ))}
                            </div>
                        ...
                        <div className='hidden lg:flex'>
                            <div className="p-2 hover:scale-110 rounded-3xl duration-200 delay-150 border-2 border-black">
                                <Link className="text-black text-center px-3 lg:px-4 text-sm lg:text-base xl:text-lg font-semibold" to="/contact">Hubungi Kami</Link>
                            </div>
                        </div>
                    </div>
                    <AnimatePresence>
                            ...
                            >
                                ...
                                >
                                    <div className="flex flex-col gap-6 sm:gap-8 justify-center mx-6 h-full text-left">
                                        {navLinks.map((link, index) => (
                                            <motion.span key={index} className={`text-black text-3xl font-bold ${activePage && activePage.to === link.to ? '' : ''}`} variants={linkItemVariants} onClick={closeModal}>
